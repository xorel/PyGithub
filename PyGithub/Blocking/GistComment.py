...
class GistComment(_bgo.UpdatableGithubObject):
    """
    ...
    def _initAttributes(self, body=_rcv.Absent, body_html=_rcv.Absent, body_text=_rcv.Absent, created_at=_rcv.Absent, id=_rcv.Absent, updated_at=_rcv.Absent, user=_rcv.Absent, **kwds):
        ...
        super(GistComment, self)._initAttributes(**kwds)
        self.__body = _rcv.Attribute("GistComment.body", _rcv.StringConverter, body)
        self.__body_html = _rcv.Attribute("GistComment.body_html", _rcv.StringConverter, body_html)
        self.__body_text = _rcv.Attribute("GistComment.body_text", _rcv.StringConverter, body_text)
        self.__created_at = _rcv.Attribute("GistComment.created_at", _rcv.DatetimeConverter, created_at)
        self.__id = _rcv.Attribute("GistComment.id", _rcv.IntConverter, id)
        self.__updated_at = _rcv.Attribute("GistComment.updated_at", _rcv.DatetimeConverter, updated_at)
        self.__user = _rcv.Attribute("GistComment.user", _rcv.ClassConverter(self.Session, PyGithub.Blocking.User.User), user)
    ...
    def _updateAttributes(self, eTag, body=_rcv.Absent, body_html=_rcv.Absent, body_text=_rcv.Absent, created_at=_rcv.Absent, id=_rcv.Absent, updated_at=_rcv.Absent, user=_rcv.Absent, **kwds):
        super(GistComment, self)._updateAttributes(eTag, **kwds)
    ...
    def delete(self):
        """
        Calls the `DELETE /gists/:gist_id/comments/:id <http://developer.github.com/v3/gists/comments#delete-a-comment>`__ end point.
    ...
    def edit(self, body):
        """
        Calls the `PATCH /gists/:gist_id/comments/:id <http://developer.github.com/v3/gists/comments#edit-a-comment>`__ end point.
